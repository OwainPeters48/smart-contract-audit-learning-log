...
contract ThunderLoan is Initializable, OwnableUpgradeable, UUPSUpgradeable, OracleUpgradeable {
    ...
    // q what happens if we deploy the contract, and someone elsde initialises it?
    // a that would suck
    // They could pick a different tswapAddress!!!!
    // @audit low - initialisers can be front run
    function initialize(address tswapAddress) external initializer {
        __Ownable_init(msg.sender);
        __UUPSUpgradeable_init();
        __Oracle_init(tswapAddress); // e using tswap as some kind of oracle?
        s_feePrecision = 1e18;
        s_flashLoanFee = 3e15; // 0.3% ETH fee
    }

    // @audit-info where is the natspec???? Important Function!!!!
